...
try:
    ...
    from botgame.training.alphastar.features import live_event_to_features
    from botgame.training.alphastar.model import FactorizedPolicyValueNet
except Exception:  # pragma: no cover
    torch = None
    FactorizedActionCodec = None
...
TELEMETRY_HP_MIN_PRESENT_RATE = 0.98
TELEMETRY_COMPLETE_MIN_RATE = 0.98
TELEMETRY_STALE_SEC = 2.5
...
def _bump_counter(counter: Dict[str, int], key: Any) -> None:
    key_s = str(key or "none").strip() or "none"
    counter[key_s] = int(counter.get(key_s, 0) or 0) + 1


def create_runtime_telemetry_monitor() -> Dict[str, Any]:
    return {
        "ticks_total": 0,
        "hp_present_ticks": 0,
        "complete_ticks": 0,
        "damage_done_nonzero_ticks": 0,
        "damage_taken_nonzero_ticks": 0,
        "damage_done_regressions": 0,
        "damage_taken_regressions": 0,
        "health_source_counts": {},
        "damage_done_source_counts": {},
        "damage_taken_source_counts": {},
    }


def build_runtime_telemetry_snapshot(
    signals: Dict[str, Any],
    now_mono: float,
    previous: Optional[Dict[str, Any]] = None,
    stale_after_sec: float = TELEMETRY_STALE_SEC,
) -> Dict[str, Any]:
    prev = previous or {}
    eps = 1e-6
    stale_ms = max(0.2, float(stale_after_sec)) * 1000.0

    prev_done = max(0.0, _finite_float_or_none(prev.get("damage_done_total")) or 0.0)
    prev_taken = max(0.0, _finite_float_or_none(prev.get("damage_taken_total")) or 0.0)
    prev_health = _finite_float_or_none(prev.get("health_current"))
    prev_health_max = _finite_float_or_none(prev.get("health_max"))

    raw_done = _finite_float_or_none(signals.get("damage_done_total"))
    raw_taken = _finite_float_or_none(signals.get("damage_taken_total"))
    if raw_done is None:
        done_total = prev_done
        done_regressed = False
    else:
        done_total = max(0.0, raw_done)
        done_regressed = done_total + eps < prev_done
        if done_regressed:
            done_total = prev_done
    if raw_taken is None:
        taken_total = prev_taken
        taken_regressed = False
    else:
        taken_total = max(0.0, raw_taken)
        taken_regressed = taken_total + eps < prev_taken
        if taken_regressed:
            taken_total = prev_taken

    raw_health_max = _finite_float_or_none(signals.get("health_max"))
    if raw_health_max is not None and raw_health_max > 0.0:
        health_max = float(raw_health_max)
    elif prev_health_max is not None and prev_health_max > 0.0:
        health_max = float(prev_health_max)
    else:
        health_max = 100.0

    done_source_sig = str(signals.get("damage_done_source", "none") or "none").strip() or "none"
    taken_source_sig = str(signals.get("damage_taken_source", "none") or "none").strip() or "none"
    health_source_sig = str(signals.get("health_source", "none") or "none").strip() or "none"

    if raw_done is None:
        done_source = "carry_forward" if prev else "none"
    else:
        done_source = done_source_sig if done_source_sig != "none" else "signal"
        if done_regressed:
            done_source = f"{done_source}_clamped"

    if raw_taken is None:
        taken_source = "carry_forward" if prev else "none"
    else:
        taken_source = taken_source_sig if taken_source_sig != "none" else "signal"
        if taken_regressed:
            taken_source = f"{taken_source}_clamped"

    raw_health = _finite_float_or_none(signals.get("health_current"))
    if raw_health is not None:
        health_current = max(0.0, min(health_max, float(raw_health)))
        health_source = health_source_sig if health_source_sig != "none" else "signal"
    else:
        can_derive = bool(
            (raw_health_max is not None and raw_health_max > 0.0)
            or (prev_health_max is not None and prev_health_max > 0.0)
            or (taken_total > 0.0)
            or bool(prev)
        )
        if can_derive:
            health_current = max(0.0, min(health_max, health_max - taken_total))
            health_source = "derived_from_damage_taken"
        elif prev_health is not None:
            health_current = max(0.0, min(health_max, float(prev_health)))
            health_source = "carry_forward"
        else:
            health_current = float(health_max)
            health_source = "default_assumed_full"

    health_sig_ts = _finite_float_or_none(signals.get("health_signal_ts"))
    done_sig_ts = _finite_float_or_none(signals.get("damage_done_signal_ts"))
    taken_sig_ts = _finite_float_or_none(signals.get("damage_taken_signal_ts"))
    health_age_ms = (
        max(0.0, (float(now_mono) - float(health_sig_ts)) * 1000.0)
        if health_sig_ts is not None and health_sig_ts > 0.0
        else None
    )
    done_age_ms = (
        max(0.0, (float(now_mono) - float(done_sig_ts)) * 1000.0)
        if done_sig_ts is not None and done_sig_ts > 0.0
        else None
    )
    taken_age_ms = (
        max(0.0, (float(now_mono) - float(taken_sig_ts)) * 1000.0)
        if taken_sig_ts is not None and taken_sig_ts > 0.0
        else None
    )

    health_stale = bool(
        (health_age_ms is not None)
        and (health_age_ms > stale_ms)
        and (health_source in {"signal", "event", "ocr", "health_drop_infer"})
    )
    done_stale = bool((done_age_ms is not None) and (done_age_ms > stale_ms))
    taken_stale = bool((taken_age_ms is not None) and (taken_age_ms > stale_ms))
    hp_present = health_current is not None
    complete = bool(hp_present)

    return {
        "health_current": float(health_current),
        "health_max": float(max(1.0, health_max)),
        "damage_done_total": float(done_total),
        "damage_taken_total": float(taken_total),
        "health_source": health_source,
        "damage_done_source": done_source,
        "damage_taken_source": taken_source,
        "health_age_ms": health_age_ms,
        "damage_done_age_ms": done_age_ms,
        "damage_taken_age_ms": taken_age_ms,
        "health_stale": health_stale,
        "damage_done_stale": done_stale,
        "damage_taken_stale": taken_stale,
        "hp_present": bool(hp_present),
        "complete": bool(complete),
        "damage_done_nonzero": bool(done_total > eps),
        "damage_taken_nonzero": bool(taken_total > eps),
        "damage_done_regressed": bool(done_regressed),
        "damage_taken_regressed": bool(taken_regressed),
    }


def update_runtime_telemetry_monitor(monitor: Dict[str, Any], snapshot: Dict[str, Any]) -> None:
    monitor["ticks_total"] = int(monitor.get("ticks_total", 0) or 0) + 1
    if bool(snapshot.get("hp_present", False)):
        monitor["hp_present_ticks"] = int(monitor.get("hp_present_ticks", 0) or 0) + 1
    if bool(snapshot.get("complete", False)):
        monitor["complete_ticks"] = int(monitor.get("complete_ticks", 0) or 0) + 1
    if bool(snapshot.get("damage_done_nonzero", False)):
        monitor["damage_done_nonzero_ticks"] = int(monitor.get("damage_done_nonzero_ticks", 0) or 0) + 1
    if bool(snapshot.get("damage_taken_nonzero", False)):
        monitor["damage_taken_nonzero_ticks"] = int(monitor.get("damage_taken_nonzero_ticks", 0) or 0) + 1
    if bool(snapshot.get("damage_done_regressed", False)):
        monitor["damage_done_regressions"] = int(monitor.get("damage_done_regressions", 0) or 0) + 1
    if bool(snapshot.get("damage_taken_regressed", False)):
        monitor["damage_taken_regressions"] = int(monitor.get("damage_taken_regressions", 0) or 0) + 1

    for key, source_key in (
        ("health_source_counts", "health_source"),
        ("damage_done_source_counts", "damage_done_source"),
        ("damage_taken_source_counts", "damage_taken_source"),
    ):
        counter = monitor.get(key)
        if not isinstance(counter, dict):
            counter = {}
            monitor[key] = counter
        _bump_counter(counter, snapshot.get(source_key, "none"))


def summarize_runtime_telemetry_monitor(
    monitor: Dict[str, Any],
    latest_snapshot: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    ticks = int(monitor.get("ticks_total", 0) or 0)
    denom = max(1, ticks)
    hp_present_ticks = int(monitor.get("hp_present_ticks", 0) or 0)
    complete_ticks = int(monitor.get("complete_ticks", 0) or 0)
    damage_done_nonzero_ticks = int(monitor.get("damage_done_nonzero_ticks", 0) or 0)
    damage_taken_nonzero_ticks = int(monitor.get("damage_taken_nonzero_ticks", 0) or 0)
    damage_done_regressions = int(monitor.get("damage_done_regressions", 0) or 0)
    damage_taken_regressions = int(monitor.get("damage_taken_regressions", 0) or 0)

    hp_present_rate = float(hp_present_ticks) / float(denom)
    complete_rate = float(complete_ticks) / float(denom)
    damage_done_nonzero_rate = float(damage_done_nonzero_ticks) / float(denom)
    damage_taken_nonzero_rate = float(damage_taken_nonzero_ticks) / float(denom)

    acceptance_pass = bool(
        ticks > 0
        and hp_present_rate >= TELEMETRY_HP_MIN_PRESENT_RATE
        and complete_rate >= TELEMETRY_COMPLETE_MIN_RATE
        and damage_done_regressions == 0
        and damage_taken_regressions == 0
    )

    return {
        "ticks_total": ticks,
        "hp_present_ticks": hp_present_ticks,
        "complete_ticks": complete_ticks,
        "damage_done_nonzero_ticks": damage_done_nonzero_ticks,
        "damage_taken_nonzero_ticks": damage_taken_nonzero_ticks,
        "hp_present_rate": round(hp_present_rate, 4),
        "complete_rate": round(complete_rate, 4),
        "damage_done_nonzero_rate": round(damage_done_nonzero_rate, 4),
        "damage_taken_nonzero_rate": round(damage_taken_nonzero_rate, 4),
        "damage_done_regressions": damage_done_regressions,
        "damage_taken_regressions": damage_taken_regressions,
        "health_source_counts": dict(monitor.get("health_source_counts", {}) or {}),
        "damage_done_source_counts": dict(monitor.get("damage_done_source_counts", {}) or {}),
        "damage_taken_source_counts": dict(monitor.get("damage_taken_source_counts", {}) or {}),
        "acceptance": {
            "hp_present_rate_min": TELEMETRY_HP_MIN_PRESENT_RATE,
            "complete_rate_min": TELEMETRY_COMPLETE_MIN_RATE,
            "damage_regressions_max": 0,
            "pass": acceptance_pass,
        },
        "latest": (
            {
                "health_current": float(latest_snapshot.get("health_current", 0.0) or 0.0),
                "health_max": float(latest_snapshot.get("health_max", 100.0) or 100.0),
                "damage_done_total": float(latest_snapshot.get("damage_done_total", 0.0) or 0.0),
                "damage_taken_total": float(latest_snapshot.get("damage_taken_total", 0.0) or 0.0),
                "health_source": str(latest_snapshot.get("health_source", "none") or "none"),
                "damage_done_source": str(latest_snapshot.get("damage_done_source", "none") or "none"),
                "damage_taken_source": str(latest_snapshot.get("damage_taken_source", "none") or "none"),
            }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=45,
        help=(
            "Maximo de espera bloqueante por paso de movimiento (ms). "
            "Permite reaccion y decision en milisegundos aunque el hold sea mayor."
        ),
    )
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Intervalo en ms para capturar feedback durante hold cooperativo.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="artifacts/alphastar/pi_rl.pt",
        help="Ruta del checkpoint AlphaStar para backend --bot-decision-backend alphastar.",
    )
    parser.add_argument(
        "--bot-alphastar-device",
        choices=("auto", "cpu", "cuda"),
        default="auto",
        help="Device para inferencia AlphaStar (auto|cpu|cuda).",
    )
    parser.add_argument(
        "--bot-alphastar-move-eps",
        type=float,
        default=0.16,
        help="Umbral minimo de magnitud para mapear move_x/move_y a teclas.",
    )
    parser.add_argument(
        "--bot-alphastar-stochastic",
        action="store_true",
        help="Si se activa, samplea acciones AlphaStar desde softmax (exploracion) en vez de argmax.",
    )
    parser.add_argument(
        "--bot-alphastar-temperature",
        type=float,
        default=1.0,
        help="Temperatura para muestreo AlphaStar (solo con --bot-alphastar-stochastic).",
    )
    parser.add_argument(
        "--bot-debug-hud",
        dest="bot_debug_hud",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Intervalo de screenshots en ms (si > 0, sobreescribe --bot-feedback-screenshot-every-sec).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--bot-feedback-max-burst-per-loop",
        type=int,
        default=4,
        help=(
            "Cantidad maxima de screenshots de catch-up por ciclo del bot "
            "(permite elevar tasa de captura cuando el loop es lento)."
        ),
    )
    parser.add_argument(
        "--bot-visual-ocr",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.25,
        help="Timeout maximo por llamada OCR (segundos) para evitar bloquear el loop.",
    )
    parser.add_argument(
        "--bot-visual-ocr-max-rois",
        type=int,
        default=1,
        help="Cantidad de regiones OCR por frame (menor valor = menor latencia).",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if bool(args.play_game) and bool(args.bot_realtime_mode):
        ...
        args.bot_ui_poll_ms = int(min(int(args.bot_ui_poll_ms), 90))
        args.bot_enemy_vision_interval_ms = int(min(int(args.bot_enemy_vision_interval_ms), 120))
    ...
    bot_event_signals.update({
        ...
        "damage_done_source": "none",
        "damage_taken_source": "none",
        "damage_done_signal_ts": 0.0,
        "damage_taken_signal_ts": 0.0,
        "health_current": None,
        "health_max": None,
        "health_source": "none",
        ...
        "enemy_guardian": "",
        "enemy_signal_ts": 0.0,
        "enemy_signal_source": "none",
        "enemy_signal_confidence": 0.0,
    ...
    def accumulate_metric_with_reset(
        metric_key: str,
        latest_value: float,
        source: str = "signal",
        ...
        if metric_key in {"damage_done", "damage_taken"}:
            bot_event_signals[f"{metric_key}_source"] = str(source or "signal")
            bot_event_signals[f"{metric_key}_signal_ts"] = float(
                time.monotonic() if now_mono is None else now_mono
            )

    def update_bot_metrics_from_event(event_obj: Dict[str, Any]) -> None:
        event_name_raw = str(event_obj.get("eventName", "") or event_obj.get("event_name", "") or "")
        event_name_l = event_name_raw.lower()
        event_name_compact = re.sub(r"[^a-z0-9]+", "", event_name_l)
        ...
        for path, value in pairs:
            ...
            if isinstance(value, str):
                ...
                if any(tok in path_l for tok in guardian_tokens):
                    if any(h in path_l for h in ("enemy", "opponent", "killer", "attacker", "target")):
                        ...
                        now_enemy = time.monotonic()
                        bot_event_signals["enemy_signal_ts"] = now_enemy
                        bot_event_signals["enemy_signal_source"] = "event_guardian"
                        bot_event_signals["enemy_signal_confidence"] = max(
                            0.62,
                            float(bot_event_signals.get("enemy_signal_confidence", 0.0) or 0.0),
                        )
                    else:
                ...
                if any(tok in path_l for tok in attacker_tokens) and len(value_s) >= 2:
                    ...
                    now_enemy = time.monotonic()
                    bot_event_signals["enemy_signal_ts"] = now_enemy
                    bot_event_signals["enemy_signal_source"] = "event_attacker"
                    bot_event_signals["enemy_signal_confidence"] = max(
                        0.70,
                        float(bot_event_signals.get("enemy_signal_confidence", 0.0) or 0.0),
                    )
            ...
            damage_done_field = (
                any(h in path_l for h in ("damage_done", "damage_dealt", "dmg_done", "damageout"))
                or any(
                    h in compact_path
                    for h in (
                        "damagedone",
            ...
            damage_taken_field = (
                any(h in path_l for h in ("damage_taken", "damage_received", "dmg_taken", "damagein"))
                or any(
                    h in compact_path
                    for h in (
                        "damagetaken",
            ...
            if (not damage_done_field) and (not damage_taken_field) and num >= 0.0:
                generic_damage_value = (
                    ("damage" in compact_path)
                    or compact_path.endswith("amount")
                    or compact_path.endswith("value")
                )
                if generic_damage_value:
                    context = f"{event_name_compact}|{compact_path}"
                    done_hints = (
                        "damagedealt",
                        "dealtdamage",
                        "damageout",
                    ...
                    has_done_hint = any(h in context for h in done_hints)
                    has_taken_hint = any(h in context for h in taken_hints)
                    if has_done_hint and (not has_taken_hint):
                        damage_done_field = True
                    elif has_taken_hint:
                        damage_taken_field = True
            if damage_done_field:
                if num >= 0:
                    accumulate_metric_with_reset("damage_done", num, source="event", now_mono=now_mono)
                    bot_event_signals["enemy_signal_ts"] = now_mono
                    bot_event_signals["enemy_signal_source"] = "event_damage_done"
                    bot_event_signals["enemy_signal_confidence"] = max(
                        0.72,
                        float(bot_event_signals.get("enemy_signal_confidence", 0.0) or 0.0),
                    )
            if damage_taken_field:
                if num >= 0:
                    accumulate_metric_with_reset("damage_taken", num, source="event", now_mono=now_mono)

            health_max_field = (
                any(h in path_l for h in ("max_health", "health_max", "max_hp", "hp_max"))
                or any(h in compact_path for h in ("maxhealth", "healthmax", "maxhp", "hpmax"))
            )
            health_current_field = (
                any(h in path_l for h in ("health", "hp"))
                or any(h in compact_path for h in ("health", "hp", "currenthealth", "currenthp"))
            )
            if health_max_field and num > 0:
                bot_event_signals["health_max"] = num
                bot_event_signals["health_source"] = "event"
                bot_event_signals["health_signal_ts"] = now_mono
            elif health_current_field and num >= 0:
                prev_health = bot_event_signals.get("health_current")
                bot_event_signals["health_current"] = num
                bot_event_signals["health_source"] = "event"
                bot_event_signals["health_signal_ts"] = now_mono
                # Fallback: infer incoming damage from health drops when no explicit damage signal arrives.
                if prev_health is not None:
                    prev_health_num = float(prev_health)
                    drop = prev_health_num - num
                    if drop > 0:
                        last_taken_signal_ts = float(bot_event_signals.get("damage_taken_signal_ts", 0.0) or 0.0)
                        if (now_mono - last_taken_signal_ts) > 0.35:
                            max_health_now = float(bot_event_signals.get("health_max", 100.0) or 100.0)
                            if drop <= max(200.0, max_health_now * 1.5):
                                bot_event_signals["damage_taken_total"] = (
                                    float(bot_event_signals.get("damage_taken_total", 0.0) or 0.0) + drop
                                )
                                bot_event_signals["damage_taken_source"] = "health_drop_infer"
                                bot_event_signals["damage_taken_signal_ts"] = now_mono
    ...
    def frame_point_to_page_target(
        ...
        iframe_box = get_iframe_box_fast(page_obj, iframe_selector, timeout_ms=90)
    ...
    def get_iframe_box_fast(
        page_obj: Page,
        iframe_selector: str,
        timeout_ms: int = 120,
    ) -> Optional[Dict[str, float]]:
        try:
            locator = page_obj.locator(iframe_selector).first
            try:
                box = locator.bounding_box(timeout=max(20, int(timeout_ms)))
            except TypeError:
                box = locator.bounding_box()
    ...
    def maybe_capture_feedback_screenshot(
        ...
        burst_cap = max(1, int(max_burst))
        current = int(session.get("shot_count", 0))
        if current >= max_shots:
            return None
        next_at = float(session.get("next_shot_at", 0.0) or 0.0)
        if next_at <= 0.0:
            next_at = now_mono
            session["next_shot_at"] = next_at
        if now_mono < next_at:
            return None
        elapsed = max(0.0, now_mono - next_at)
        captures_to_take = 1 + int(elapsed // max(0.001, interval))
        captures_to_take = max(1, min(captures_to_take, burst_cap))
        safe_label = "".join(ch if ch.isalnum() or ch in ("_", "-") else "_" for ch in str(label or "tick"))
        last_path: Optional[Path] = None
        for burst_idx in range(captures_to_take):
            current = int(session.get("shot_count", 0))
            if current >= max_shots:
                break
            shot_index = current + 1
            suffix = f"_b{burst_idx+1}" if captures_to_take > 1 else ""
            filename = f"{shot_index:04d}_{safe_label}{suffix}.png"
            ...
            try:
                ...
                shot_dt_ms = (time.monotonic() - shot_started_at) * 1000.0
                if shot_dt_ms >= 350.0:
                    print(f"[BOT][LATENCY][SLOW] section=feedback_screenshot dt_ms={shot_dt_ms:.1f}")
            except Exception as exc:
    ...
    def cooperative_hold_with_feedback(
        ...
        max_burst: int,
        label: str,
        max_blocking_hold_ms: int,
        hold_capture_slice_ms: int,
        full_page: bool = False,
    ) -> Optional[str]:
        hold_total = max(0.0, float(hold_seconds))
        if hold_total <= 0.0:
            return None
        max_blocking_sec = max(0.0, float(max_blocking_hold_ms) / 1000.0)
        effective_hold = hold_total if max_blocking_sec <= 0.0 else min(hold_total, max_blocking_sec)
        if effective_hold <= 0.0:
            return None
        slice_sec = max(0.005, float(max(1, int(hold_capture_slice_ms))) / 1000.0)
        hold_until = time.monotonic() + effective_hold
        last_path: Optional[str] = None
        while True:
            now_tick = time.monotonic()
            if now_tick >= hold_until:
                break
    ...
    def alphastar_action_to_move_combo(action_obj: Any, eps: float = 0.16) -> List[str]:
        keys: List[str] = []
        try:
            move_x = float(getattr(action_obj, "move_x", 0.0) or 0.0)
            move_y = float(getattr(action_obj, "move_y", 0.0) or 0.0)
    ...
    def load_alphastar_live_policy(checkpoint_path: str, device_pref: str = "auto") -> Optional[Dict[str, Any]]:
        if (
            torch is None
            or FactorizedActionCodec is None
            or FactorizedPolicyValueNet is None
            or live_event_to_features is None
        ):
            return None
        ckpt_file = Path(str(checkpoint_path or "").strip())
        if not ckpt_file.exists():
            return None
        try:
            if str(device_pref).strip().lower() == "cpu":
                device = torch.device("cpu")
            elif str(device_pref).strip().lower() == "cuda":
        ...
        obs_dim = int(ckpt.get("obs_dim", 19))
        head_sizes = dict(ckpt.get("head_sizes", {}))
        if not head_sizes:
            return None
        model = FactorizedPolicyValueNet(obs_dim=obs_dim, head_sizes=head_sizes)
    ...
    def build_alphastar_live_event_snapshot(
        now_mono: float,
        bot_state: str,
        ui_state: Dict[str, Any],
        active_keys_runtime: Set[str],
        input_probe_now: Dict[str, Any],
        click_probe_now: Dict[str, Any],
        cursor_probe_now: Dict[str, Any],
        last_action_ok: bool,
    ) -> Dict[str, Any]:
        return {
            "mono": now_mono,
            ...
            "active_keys": sorted(active_keys_runtime) if active_keys_runtime else [],
            "input_probe": dict(input_probe_now or {}),
            "click_probe": dict(click_probe_now or {}),
            "cursor_probe": dict(cursor_probe_now or {}),
    ...
    def capture_canvas_motion_frame(
        ...
        try:
            ...
            cap_dt_ms = (time.monotonic() - cap_started_at) * 1000.0
            if cap_dt_ms >= 350.0:
                print(f"[BOT][LATENCY][SLOW] section=motion_capture dt_ms={cap_dt_ms:.1f}")
    ...
    try:
        with sync_playwright() as p:
            ...
            if run_play_game:
                ...
                feedback_screenshot_every_ms = max(
                    0,
                    int(getattr(args, "bot_feedback_screenshot_every_ms", 0) or 0),
                )
                if feedback_screenshot_every_ms > 0:
                    feedback_screenshot_interval_sec = max(
                        0.01,
                        float(feedback_screenshot_every_ms) / 1000.0,
                    )
                if runtime_feedback_session:
                    ...
                    append_feedback_event(
                        runtime_feedback_session,
                        {
                            ...
                            "feedback_screenshot_interval_ms": int(round(feedback_screenshot_interval_sec * 1000.0)),
                            "move_hold_base_ms": int(args.bot_move_base_hold_ms),
                ...
                alphastar_runtime: Optional[Dict[str, Any]] = None
                alphastar_last_error_at = 0.0
                if bot_decision_backend == "lms_re":
                    if LMSReverseEngineeredBot is None:
                        ...
                        try:
                            ...
                            lms_re_policy = None
                            bot_decision_backend = "legacy"
                elif bot_decision_backend == "alphastar":
                    alphastar_runtime = load_alphastar_live_policy(
                        checkpoint_path=str(getattr(args, "bot_alphastar_checkpoint", "") or ""),
                        device_pref=str(getattr(args, "bot_alphastar_device", "auto") or "auto"),
                    )
                    if alphastar_runtime is None:
                        print(
                            "[BOT][DECISION][WARN] backend=alphastar no disponible "
                            f"(checkpoint={str(getattr(args, 'bot_alphastar_checkpoint', '') or '')}); fallback a legacy."
                        )
                        bot_decision_backend = "legacy"
                    else:
                        print(
                            "[BOT][DECISION] backend=alphastar "
                            f"checkpoint={alphastar_runtime.get('checkpoint')} "
                            f"device={alphastar_runtime.get('device')}"
                        )
                else:
                ...
                loop_prev_tick_mono = 0.0
                while True:
                    now_mono = time.monotonic()
                    if loop_prev_tick_mono > 0.0:
                        loop_dt_ms = (now_mono - loop_prev_tick_mono) * 1000.0
                        if loop_dt_ms >= 1200.0:
                            print(f"[BOT][LATENCY][WARN] loop_dt_ms={loop_dt_ms:.1f}")
                    loop_prev_tick_mono = now_mono
                    ...
                    lms_re_ability_key = ""
                    lms_re_action_desc = ""
                    alphastar_move_combo: List[str] = []
                    alphastar_ability_key = ""
                    alphastar_should_fire = False
                    alphastar_action_desc = ""
                    hold_capture_path: Optional[str] = None
                    ...
                    if parallel_smoke_state is not None:
                        ...
                        if (now_mono - float(last_bring_to_front_at)) >= 2.0:
                            try:
                                page.bring_to_front()
                                last_bring_to_front_at = now_mono
                            except Exception:
                                pass
                    ...
                    should_scan_play_target = bool(
                        ui_state.get("canvas_visible")
                        and (
                            (last_state != "in_match")
                    ...
                    if bot_state == "lobby":
                        ...
                        if bool(args.bot_enemy_vision) and (
                            ...
                            if bool(enemy_signal_cache.get("detected", False)):
                                ...
                                bot_event_signals["enemy_signal_ts"] = now_mono
                                bot_event_signals["enemy_signal_source"] = "vision"
                        ...
                        enemy_event_ts = float(bot_event_signals.get("enemy_signal_ts", 0.0) or 0.0)
                        enemy_event_recent = bool(enemy_event_ts > 0.0 and ((now_mono - enemy_event_ts) <= 1.8))
                        enemy_event_conf = float(bot_event_signals.get("enemy_signal_confidence", 0.0) or 0.0)
                        ...
                        if (not bool(enemy_signal_cache.get("detected", False))) and enemy_conf_ema < 0.18:
                            ...
                            enemy_recent = False
                        if enemy_event_recent and (not enemy_recent):
                            enemy_recent = True
                            enemy_signal_cache["detected"] = True
                            enemy_signal_cache["confidence"] = max(
                                float(enemy_signal_cache.get("confidence", 0.0) or 0.0),
                                enemy_event_conf,
                                0.45,
                            )
                            enemy_signal_cache["dir"] = str(enemy_signal_cache.get("dir", "CENTER") or "CENTER")
                            enemy_signal_cache["direction_key"] = str(
                                enemy_signal_cache.get("direction_key", "KeyW") or "KeyW"
                            )
                        enemy_for_feedback = dict(enemy_signal_cache)
                        enemy_for_feedback["recent"] = enemy_recent
                        enemy_for_feedback["event_recent"] = enemy_event_recent
                        enemy_for_feedback["event_source"] = str(
                            bot_event_signals.get("enemy_signal_source", "none") or "none"
                        )
                        ...
                        elif bot_decision_backend == "alphastar" and alphastar_runtime is not None:
                            try:
                                model = alphastar_runtime["model"]
                                codec = alphastar_runtime["codec"]
                                infer_device = alphastar_runtime["device"]
                                obs_dim_expected = int(alphastar_runtime.get("obs_dim", 19))
                                infer_event = build_alphastar_live_event_snapshot(
                                    now_mono=now_mono,
                                    bot_state=bot_state,
                                    ui_state=ui_state,
                                    active_keys_runtime=active_keys_runtime,
                                    input_probe_now=input_probe_now,
                                    click_probe_now=click_probe_now,
                                    cursor_probe_now=cursor_probe_now,
                                    last_action_ok=last_action_ok,
                                )
                                feat_vec = live_event_to_features(infer_event)
                                if feat_vec.shape[0] != obs_dim_expected:
                                    fixed = np.zeros((obs_dim_expected,), dtype=np.float32)
                                    copy_n = min(obs_dim_expected, int(feat_vec.shape[0]))
                                    fixed[:copy_n] = feat_vec[:copy_n]
                                    feat_vec = fixed
                                obs_tensor = torch.as_tensor(feat_vec, dtype=torch.float32, device=infer_device).unsqueeze(0)
                                with torch.no_grad():
                                    alpha_out = model(obs_tensor)
                                encoded: Dict[str, int] = {}
                                stochastic_alpha = bool(getattr(args, "bot_alphastar_stochastic", False))
                                temperature_alpha = max(
                                    0.05,
                                    float(getattr(args, "bot_alphastar_temperature", 1.0) or 1.0),
                                )
                                for head_name, head_logits in alpha_out.logits.items():
                                    if stochastic_alpha:
                                        logits = head_logits / temperature_alpha
                                        probs = torch.softmax(logits, dim=-1)
                                        sampled = torch.multinomial(probs.squeeze(0), num_samples=1)
                                        encoded[head_name] = int(sampled.item())
                                    else:
                                        encoded[head_name] = int(torch.argmax(head_logits, dim=-1).item())
                                alpha_action = codec.decode_action(encoded)
                                alphastar_move_combo = alphastar_action_to_move_combo(
                                    alpha_action,
                                    eps=float(getattr(args, "bot_alphastar_move_eps", 0.16)),
                                )
                                alphastar_ability_key = lms_re_action_to_ability_key(alpha_action)
                                alphastar_should_fire = bool(getattr(alpha_action, "fire", False))
                                alphastar_action_desc = (
                                    f"mv={'+'.join(alphastar_move_combo or ['-'])} "
                                    f"ab={alphastar_ability_key or '-'} "
                                    f"fire={1 if alphastar_should_fire else 0}"
                                )
                            except Exception as exc:
                                if (now_mono - alphastar_last_error_at) >= 4.0:
                                    print(f"[BOT][DECISION][WARN] alphastar act fallo: {exc}")
                                    alphastar_last_error_at = now_mono
                        ...
                        if forced_move_queue:
                        ...
                        elif opening_phase_active and (not enemy_recent) and (not escape_mode) and (bot_decision_backend not in ("lms_re", "alphastar")):
                            ...
                            if alphastar_move_combo:
                                alpha_combo_norm = normalize_move_combo(list(alphastar_move_combo))
                                if zone_escape_mode:
                                    move_candidates.append(alpha_combo_norm)
                                else:
                                    move_candidates.insert(0, alpha_combo_norm)
                                move_policy_score = max(move_policy_score, 0.22)
                        ...
                        backend_action_desc = lms_re_action_desc or alphastar_action_desc or "-"
                        print(
                            "[BOT][MOVE_SMOKE] "
                        ...
                        try:
                            ...
                            hold_capture_path = cooperative_hold_with_feedback(
                                ...
                                max_blocking_hold_ms=int(args.bot_move_max_blocking_hold_ms),
                                hold_capture_slice_ms=int(args.bot_hold_capture_slice_ms),
                                full_page=False,
                            )
                            if sprint_engaged:
                                try:
                        ...
                        if (
                            bot_decision_backend == "alphastar"
                            and alphastar_ability_key in ("Digit1", "Digit2", "Digit3", "KeyR")
                        ):
                            base_ability_priority = [alphastar_ability_key] + [
                                key for key in base_ability_priority if key != alphastar_ability_key
                            ]
                        ...
                        policy_fire_signal = bool(
                            (bot_decision_backend == "alphastar")
                            and bool(alphastar_should_fire)
                            and (not opening_phase_active)
                        )
                        enemy_detected_for_attack = bool(enemy_recent or enemy_event_recent)
                        attack_due = bool(
                            bool(enemy_signal_cache.get("near", False))
                            or ((smoke_move_index % attack_every_steps) == 0)
                            or (policy_fire_signal and enemy_detected_for_attack)
                        )
                        should_attack = bool(enemy_detected_for_attack and attack_due and (not zone_escape_mode))
                        attack_target = None
                        ...
                        if should_attack and (attack_target is not None):
                            ...
                            if ok_click:
                                ...
                                print(
                                    ...
                                    f"enemy_src={enemy_for_feedback.get('event_source', 'vision')}, "
                                    f"fire={1 if policy_fire_signal else 0})"
                                )
                            else:
                            ...
                            if bool(visual_ocr_now.get("ok", False)):
                                ...
                                if raw_excerpt:
                                    ...
                                    if hp_match:
                                        ...
                                        if hp_now >= 0.0:
                                            bot_event_signals["health_current"] = hp_now
                                            if bot_event_signals.get("health_max") is None:
                                                bot_event_signals["health_max"] = 100.0
                                            bot_event_signals["health_source"] = "ocr"
                                            bot_event_signals["health_signal_ts"] = now_mono
                                    out_match = re.search(r"\bout\s*[:=]\s*([0-9]+(?:\.[0-9]+)?)", raw_excerpt, flags=re.IGNORECASE)
                                    if out_match:
                                        out_total = float(out_match.group(1))
                                        if out_total >= 0.0:
                                            accumulate_metric_with_reset("damage_done", out_total, source="ocr", now_mono=now_mono)
                                    in_match = re.search(r"\bin\s*[:=]\s*([0-9]+(?:\.[0-9]+)?)", raw_excerpt, flags=re.IGNORECASE)
                                    if in_match:
                    ...
                    tick_telemetry = build_runtime_telemetry_snapshot(
                        signals=bot_event_signals,
                        now_mono=now_mono,
                        previous=last_tick_telemetry,
                    )
                    update_runtime_telemetry_monitor(telemetry_monitor, tick_telemetry)
                    last_tick_telemetry = dict(tick_telemetry)

                    current_damage_done_total = float(tick_telemetry.get("damage_done_total", 0.0) or 0.0)
                    current_damage_taken_total = float(tick_telemetry.get("damage_taken_total", 0.0) or 0.0)
                    health_current_for_feedback = float(tick_telemetry.get("health_current", 0.0) or 0.0)
                    health_max_for_feedback = float(tick_telemetry.get("health_max", 100.0) or 100.0)
                    bot_event_signals["damage_done_total"] = current_damage_done_total
                    bot_event_signals["damage_taken_total"] = current_damage_taken_total
                    if bot_event_signals.get("health_current") is None:
                        bot_event_signals["health_current"] = health_current_for_feedback
                        bot_event_signals["health_source"] = str(
                            tick_telemetry.get("health_source", "derived_from_damage_taken") or "derived_from_damage_taken"
                        )
                    if bot_event_signals.get("health_max") is None:
                        bot_event_signals["health_max"] = health_max_for_feedback
                    ...
                    append_feedback_event(
                        runtime_feedback_session,
                        {
                            ...
                            "health_current": float(health_current_for_feedback),
                            "health_max": float(health_max_for_feedback),
                            "telemetry_quality": {
                                "complete": bool(tick_telemetry.get("complete", False)),
                                "hp_present": bool(tick_telemetry.get("hp_present", False)),
                                "health_source": str(tick_telemetry.get("health_source", "none") or "none"),
                                "damage_done_source": str(tick_telemetry.get("damage_done_source", "none") or "none"),
                                "damage_taken_source": str(tick_telemetry.get("damage_taken_source", "none") or "none"),
                                "health_age_ms": tick_telemetry.get("health_age_ms"),
                                "damage_done_age_ms": tick_telemetry.get("damage_done_age_ms"),
                                "damage_taken_age_ms": tick_telemetry.get("damage_taken_age_ms"),
                                "health_stale": bool(tick_telemetry.get("health_stale", False)),
                                "damage_done_stale": bool(tick_telemetry.get("damage_done_stale", False)),
                                "damage_taken_stale": bool(tick_telemetry.get("damage_taken_stale", False)),
                                "damage_done_regressed": bool(tick_telemetry.get("damage_done_regressed", False)),
                                "damage_taken_regressed": bool(tick_telemetry.get("damage_taken_regressed", False)),
                            },
                            "mana": float(ability_state.get("mana", 0.0) or 0.0),
                            "ability_last": str(ability_state.get("last_used_key", "") or ""),
                            "ability_class": str(ability_state.get("last_used_class", "") or ""),
                    ...
                    min_poll_sleep = 0.02 if bool(args.bot_realtime_mode) else 0.1
                    time.sleep(max(min_poll_sleep, float(args.bot_ui_poll_ms) / 1000.0))
                    generate_report_if_needed(page)
                if (
                    str(run_stop_reason or "").strip().lower() == "death_event"
                    and float(bot_event_signals.get("damage_taken_total", 0.0) or 0.0) <= 0.0
                ):
                    inferred_damage_taken = float(bot_event_signals.get("visual_damage_hint", 0.0) or 0.0)
                    inferred_damage_taken = max(0.0, min(1200.0, inferred_damage_taken))
                    if inferred_damage_taken <= 0.0 and (bot_event_signals.get("health_current") is not None):
                        health_max_now = float(bot_event_signals.get("health_max", 100.0) or 100.0)
                        health_now = float(bot_event_signals.get("health_current", 0.0) or 0.0)
                        inferred_damage_taken = max(0.0, health_max_now - health_now)
                    if inferred_damage_taken <= 0.0:
                        health_max_now = float(bot_event_signals.get("health_max", 100.0) or 100.0)
                        inferred_damage_taken = max(1.0, health_max_now)
                    if bot_event_signals.get("health_max") is None:
                        bot_event_signals["health_max"] = 100.0
                    bot_event_signals["health_current"] = 0.0
                    bot_event_signals["health_source"] = "death_fallback"
                    bot_event_signals["health_signal_ts"] = time.monotonic()
                    if inferred_damage_taken > 0.0:
                        bot_event_signals["damage_taken_total"] = inferred_damage_taken
                        bot_event_signals["damage_taken_latest"] = max(
                            float(bot_event_signals.get("damage_taken_latest", 0.0) or 0.0),
                            inferred_damage_taken,
                        )
                        bot_event_signals["damage_taken_source"] = "death_fallback"
                        bot_event_signals["damage_taken_signal_ts"] = time.monotonic()
                        print(
                            "[BOT][METRICS] damage_taken_total inferido "
                            f"por fallback en muerte: {inferred_damage_taken:.1f}"
                        )
                ...
                final_telemetry_snapshot = build_runtime_telemetry_snapshot(
                    signals=bot_event_signals,
                    now_mono=time.monotonic(),
                    previous=last_tick_telemetry,
                )
                bot_event_signals["damage_done_total"] = float(final_telemetry_snapshot.get("damage_done_total", 0.0) or 0.0)
                bot_event_signals["damage_taken_total"] = float(final_telemetry_snapshot.get("damage_taken_total", 0.0) or 0.0)
                bot_event_signals["health_current"] = float(final_telemetry_snapshot.get("health_current", 0.0) or 0.0)
                bot_event_signals["health_max"] = float(final_telemetry_snapshot.get("health_max", 100.0) or 100.0)
                bot_event_signals["health_source"] = str(
                    final_telemetry_snapshot.get("health_source", bot_event_signals.get("health_source", "none")) or "none"
                )
                telemetry_summary = summarize_runtime_telemetry_monitor(
                    telemetry_monitor,
                ...
                if runtime_feedback_session:
                    append_feedback_event(
                        runtime_feedback_session,
                        {
                            ...
                            "health_current": float(bot_event_signals.get("health_current", 0.0) or 0.0),
                            "health_max": float(bot_event_signals.get("health_max", 100.0) or 100.0),
                            "health_source": str(bot_event_signals.get("health_source", "none") or "none"),
                            "death": {
                                "active": bool(float(bot_event_signals.get("death_ts", 0.0) or 0.0) > 0.0),
                                "cause": str(bot_event_signals.get("death_cause", "unknown") or "unknown"),
                                "cause_confidence": float(bot_event_signals.get("death_cause_confidence", 0.0) or 0.0),
                            ...
                            "telemetry_quality": telemetry_summary,
                        },
                    )
                ...
                if args.bot_run_until_end:
                    ...
                    print(
                        ...
                        f"telemetry_pass={bool((telemetry_summary.get('acceptance', {}) or {}).get('pass', False))} "
